...
#          Katarina Piponi, Jean Louis Konan, Konan Engueran Djaha, Hala N'klo,
...
#   yield dataset and comparing them by AIC. Metrics are z-scored before
#   model fitting to ensure numerical stability across metrics of different
#   units and scales. The best-fitting metric and evidence for nonlinearity
#   (ΔAIC = AIC_lin - AIC_quad) are reported.
#
...
# --- 2c. Build random-effects string ---
...
# --- 2d. Fit linear and quadratic annual models; return AIC comparison row ---
...
cat("  No filtering | Z-standardised metrics | Linear vs Quadratic | glmmTMB nbinom2\n")
cat("=============================================================================\n\n")
...
# ==============================================================================
# 5. MODEL FITTING: Z-STANDARDISE -> LINEAR VS QUADRATIC
# ==============================================================================

cat("Fitting annual linear vs quadratic models...\n")
cat("  Metrics are z-scored before fitting for numerical stability.\n\n")
...
for (met in candidate_metrics) {
  ...
  n_trees <- length(unique(a_std[[tree_id_col]]))
  cat("  [trees:", n_trees, "| rows:", nrow(a_std), "]\n")
  
  if (n_trees < 10) {
    cat("    !! Skipped — fewer than 10 trees\n")
    next
  }
  
  result <- fit_lin_quad_annual(a_std, yield_col, re_str)
  
  if (!is.null(result)) {
    results_list[[met]] <- result %>%
      mutate(metric = met, scale = "annual", n = nrow(a_std), n_trees = n_trees)
  } else {
...
if (nrow(results) > 0) {
  ...
  summary_lines <- c(
    ...
    "",
    "Notes:",
    "  - Metrics are z-scored before fitting for numerical stability.",
    "  - No coverage filtering applied.",
    "  - AIC values are invariant to this linear transformation;",
    "    metric ranking is unaffected.",
    "",
    "Decision checklist:",
...
#
#   Three filtered datasets are produced:
#
#     main_dataset
#       : base filter — minimum data quality thresholds
#         (raw linearised slope used as phenotype)
#
#     supporting_dataset_1
#       : base filter — precision-weighted slopes
#         (slope_q_lin / slope_q_lin_se) used as phenotype
#         to assess sensitivity to heterogeneous slope precision
#
#     supporting_dataset_2
#       : stringent filter — additionally requires coverage of both the
#         dry and wet tails of the drought metric
#         (raw linearised slope used as phenotype)
#
# Inputs:
...
#     pheno_slope_q_lin_<x1>_<x2>_precision_weighted.txt
#     pipeline_summary_precision_weighted.txt
#
#   supporting_dataset_2/
#     reaction_norms_all_trees_trimmed.csv
#     reaction_norms_supporting_dataset_2.csv
#     phenotypes_sequenced_supporting_dataset_2.csv
#     pheno_slope_q_lin_<x1>_<x2>_stringent.txt
...
# --- Stringent filter (supporting_dataset_2) ---
...
# ==============================================================================
# 7. SUPPORTING DATASET 1 (BASE FILTER + PRECISION WEIGHTING)
# ==============================================================================

cat("=== Supporting dataset 1 (base filter + precision weighting) ===\n\n")

supp1_dir <- file.path(output_dir, "supporting_dataset_1")
dir.create(supp1_dir, showWarnings = FALSE, recursive = TRUE)

log_supp1 <- file.path(supp1_dir, "pipeline_summary_precision_weighted.txt")
sink(log_supp1, split = TRUE)

tree_supp1 <- tree_main %>%
  filter(is.finite(slope_q_lin_se), slope_q_lin_se > 0) %>%
  mutate(slope_q_lin_pw = slope_q_lin / slope_q_lin_se)

pheno_supp1 <- sequenced %>% inner_join(tree_supp1, by = "new_tree_id")

cat("Trees passing precision-weight filter  :", nrow(tree_supp1), "\n")
cat("Sequenced trees (supporting_dataset_1) :", nrow(pheno_supp1), "\n")

summarise_rn(tree_all, tree_supp1, annualised_mixed, "SUPPORTING DATASET 1")
...
write_pheno(pheno_supp1, "slope_q_lin_pw",
            file.path(supp1_dir, "drought_slope_estimates_precision_weighted.txt"))
write_pheno(pheno_supp1, "intercept", file.path(supp1_dir, "pheno_intercept_precision_weighted.txt"))
write_pheno(pheno_supp1, "beta1",     file.path(supp1_dir, "pheno_beta1_precision_weighted.txt"))
write_pheno(pheno_supp1, "beta2",     file.path(supp1_dir, "pheno_beta2_precision_weighted.txt"))
...
write_pheno_with_header(
  pheno_supp1, "slope_q_lin_pw",
  file.path(drought_slope_dir, "supporting_dataset_1.txt"),
  comment = paste0("REMOVE THIS TEXT BEFORE RUNNING GENOMIC ANALYSES. ",
                   "Phenotype file used in genomic analyses (GWAS). ",
                   "Precision-weighted linearised drought slope (-1 \u2264 RDI-18 \u2264 0), ",
                   "supporting dataset 1 (base filter, precision-weighted). ",
                   "Key: FID/IID = sequencing ID to match genomic data; ",
                   "PHENO = precision-weighted drought slope estimate (slope / SE).")
)
...
# ==============================================================================
# 8. SUPPORTING DATASET 2 (STRINGENT FILTER)
# ==============================================================================

cat("=== Supporting dataset 2 (stringent filter) ===\n")
...
supp2_dir <- file.path(output_dir, "supporting_dataset_2")
dir.create(supp2_dir, showWarnings = FALSE, recursive = TRUE)

log_supp2 <- file.path(supp2_dir, "pipeline_summary_stringent.txt")
sink(log_supp2, split = TRUE)
...
pheno_supp2 <- sequenced %>% inner_join(tree_supp2, by = "new_tree_id")

cat("Trees passing stringent filter         :", nrow(tree_supp2), "\n")
cat("Sequenced trees (supporting_dataset_2) :", nrow(pheno_supp2), "\n")

summarise_rn(tree_stringent_all, tree_supp2, annualised_trimmed, "SUPPORTING DATASET 2")

# Save outputs
fwrite(tree_stringent_all, file.path(supp2_dir, "reaction_norms_all_trees_trimmed.csv"))
fwrite(tree_supp2,         file.path(supp2_dir, "reaction_norms_supporting_dataset_2.csv"))
fwrite(pheno_supp2,        file.path(supp2_dir, "phenotypes_sequenced_supporting_dataset_2.csv"))

write_pheno(pheno_supp2, "slope_q_lin",
            file.path(supp2_dir, "drought_slope_estimates.txt"))
write_pheno(pheno_supp2, "intercept", file.path(supp2_dir, "pheno_intercept_stringent.txt"))
write_pheno(pheno_supp2, "beta1",     file.path(supp2_dir, "pheno_beta1_stringent.txt"))
write_pheno(pheno_supp2, "beta2",     file.path(supp2_dir, "pheno_beta2_stringent.txt"))

sink()
cat("Outputs saved to:", supp2_dir, "\n")
cat("Summary log     :", log_supp2, "\n\n")

write_pheno_with_header(
  pheno_supp2, "slope_q_lin",
  file.path(drought_slope_dir, "supporting_dataset_2.txt"),
  comment = paste0("REMOVE THIS TEXT BEFORE RUNNING GENOMIC ANALYSES. ",
                   "Phenotype file used in genomic analyses (GWAS). ",
                   "Linearised drought slope (-1 \u2264 RDI-18 \u2264 0), ",
                   "supporting dataset 2 (stringent filter). ",
                   "Key: FID/IID = sequencing ID to match genomic data; ",
                   "PHENO = drought slope estimate.")
)
...
cat("  supporting_dataset_1/  (precision-weighted slopes)  :", nrow(tree_supp1), "trees |",
    nrow(pheno_supp1), "sequenced\n")
cat("  supporting_dataset_2/  (stringent filter)           :", nrow(tree_supp2), "trees |",
    nrow(pheno_supp2), "sequenced\n")
cat("=== Script 3 complete ===\n")

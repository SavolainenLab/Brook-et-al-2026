...
stringent_min_dry       <- 3
stringent_min_wet       <- 3
stringent_min_neg_range <- 0.5
stringent_min_pos_range <- 0.5
stringent_rdi_max       <- 1000
...
# ==============================================================================
# 3. STAGE 1 — MIXED MODEL ADJUSTMENT
# ==============================================================================

mix_dat <- annualised_raw %>%
  filter(is.finite(yield_raw), is.finite(age_raw)) %>%
  mutate(age_c = scale(age_raw, center = TRUE, scale = FALSE))

m_mixed <- lmer(yield_raw ~ age_c + (1|new_tree_id) + (1|year),
                data = mix_dat, REML = TRUE)
...
# ==============================================================================
# 4. QUADRATIC REACTION NORM FUNCTION
# ==============================================================================
...
fit_trees <- function(dat) {
  dat %>%
    group_by(new_tree_id) %>%
    summarise(
      n_obs = sum(is.finite(yield_adj) &
                    is.finite(.data[[drought_metric]])),
      ...
      neg_rdi_range =
        diff(range(.data[[drought_metric]]
                   [.data[[drought_metric]]<0], na.rm=TRUE)),
      pos_rdi_range =
        diff(range(.data[[drought_metric]]
                   [.data[[drought_metric]]>=0], na.rm=TRUE)),
      tmp = list(calc_quad(yield_adj,
                           .data[[drought_metric]],
                           x1_linearise,
                           x2_linearise)),
      .groups="drop"
...
write_pheno <- function(df, trait_col, path) {
  df %>%
    transmute(FID=sequencing_id,
              IID=sequencing_id,
              PHENO=.data[[trait_col]]) %>%
...
# ==============================================================================
# 6. MAIN DATASET (BASE FILTER, RAW SLOPE)
# ==============================================================================
...
pheno_main <- sequenced %>%
  inner_join(tree_main, by="new_tree_id")
...
dir.create(supp1_dir, showWarnings=FALSE)

tree_supp1 <- tree_main %>%
  filter(is.finite(slope_q_lin_se), slope_q_lin_se > 0) %>%
  mutate(
    slope_q_lin_pw = slope_q_lin / slope_q_lin_se
  )

pheno_supp1 <- sequenced %>%
  inner_join(tree_supp1, by="new_tree_id")

write_pheno(pheno_supp1, "slope_q_lin_pw",
            file.path(supp1_dir,
                      "drought_slope_estimates_precision_weighted.txt"))

# ==============================================================================
# 8. SUPPORTING DATASET 2 (STRINGENT FILTER, RAW SLOPE)
# ==============================================================================

supp2_dir <- file.path(output_dir, "supporting_dataset_2")
dir.create(supp2_dir, showWarnings=FALSE)
...
pheno_supp2 <- sequenced %>%
  inner_join(tree_supp2, by="new_tree_id")

write_pheno(pheno_supp2, "slope_q_lin",
            file.path(supp2_dir,
                      "drought_slope_estimates.txt"))
...
cat("\nSCRIPT 3 COMPLETE\n")
cat("Main dataset trees:", nrow(tree_main), "\n")
cat("Supporting dataset 1 (precision-weighted) trees:",
    nrow(tree_supp1), "\n")
cat("Supporting dataset 2 (stringent) trees:",
    nrow(tree_supp2), "\n")
